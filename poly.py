...
def poly_result(order=5, d=0):
    product = order
    if d<order:
        return r"\infty"
    elif d==order:
        for coef in range(order - 1, 0, -1):
            product *= coef
        return str(product)
    else:
        return "0"

def poly_deriv_tex(order=5,d=0,mark_coef=True):
...
def lhpoly_tex(name='f', orders=5, d=0, mark_coef=True, mark_poly=False):
    try:
        for order in orders:
            pass
    except TypeError:
    ...
    for order in orders:
        ...
        if p!="":
            p+="+"
        p+=poly_deriv_tex(order,d,mark_coef=mark_coef)
    tex = (r"{{" + name + r"}}" + primes + r"{{(x)}} &= " +
           open_poly+p+close_poly + r"\\" +
           r" &= {{ " + poly_result(maxorder,d))+" }} "
    return tex
